...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == BotState.ENTERING_SECRET:
        await _handle_entering_secret(session, text, update)
    elif state == BotState.WAITING_WM_GUESS:
        await _handle_wm_guess(session, text, update)
    elif state == BotState.WAITING_INTENDED_WORD:
        await _handle_intended_word(session, text, update)
    elif state == BotState.WAITING_CLUE:
        await _handle_clue(session, text, update)
    elif state == BotState.WAITING_PARTNER_GUESS:
        await _handle_partner_guess(session, text, update)
    else:
        await update.message.reply_text(i18n.get("unexpected", lang))


async def _handle_entering_secret(session, text: str, update: Update) -> None:
    lang = session.language
...
async def _handle_wm_guess(session, text: str, update: Update) -> None:
    lang = session.language
...
async def _handle_intended_word(session, text: str, update: Update) -> None:
    lang = session.language
    snapshot = await session.gm.get_state()
    prefix = snapshot.currentPrefix
    used = snapshot.usedWords
...
async def _handle_clue(session, text: str, update: Update) -> None:
    lang = session.language
...
async def _handle_partner_guess(session, text: str, update: Update) -> None:
    lang = session.language
    snapshot = await session.gm.get_state()
    prefix = snapshot.currentPrefix
    used = snapshot.usedWords
